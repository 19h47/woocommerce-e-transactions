 ...
 */
class WC_E4Gw extends WC_Etransactions_Abstract_Gateway {
	protected $defaultTitle = 'E-Transactions 4 times payment';
	protected $defaultDesc  = 'xxxx';
	protected $type         = 'fourtime';

	public function __construct() {
		// Some properties
		$this->id           = 'etransactions_4x';
		$this->method_title = __( 'E-Transactions 4 times', WC_ETRANSACTIONS_PLUGIN );
		$this->has_fields   = false;
		$this->icon         = '4xcbvisamcecb.png';
	...
	public function showDetails( $order ) {
		...
		if ( ! empty( $payment ) ) {
			...
			$payment = $this->_etransactions->getOrderPayments( $order_id, 'fourth_payment' );

			if ( ! empty( $payment ) ) {
				$fourth = unserialize( $payment->data );
			}
			...
			if ( isset( $fourth ) ) {
				$date  = preg_replace( '/^([0-9]{2})([0-9]{2})([0-9]{4})$/', '$1/$2/$3', $third['date'] );
				$value = sprintf( '%s (%s)', $fourth['amount'] / 100.0, $date );
			} else {
				$value = __( 'Not achieved', WC_ETRANSACTIONS_PLUGIN );
			}

			$rows[] = $this->_showDetailRow( __( 'Fourth debit:' ), $value );
...
class WC_Etransactions_FourTime_GateWay extends WC_E4Gw {
	public function is_available() {
			return false;
	}
